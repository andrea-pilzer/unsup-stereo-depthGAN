...
parser.add_argument('--min_depth',           type=float, help='minimum depth for evaluation',        default=0.001)
...
if __name__ == '__main__':
    ...
    if args.split == 'kitti':
        ...
        num_samples = 1525
        test_files = read_text_lines('/data1/pilzer/monodepth-GAN/utils/filenames/cityscapes_test_files.txt')
        #read disparity files
        gt_files = []
        im_files = []
        im_sizes = []
        cams = []
        ...
        for filename in test_files:
            ...
            split = filename.rsplit('_',1)[0] + '_disparity.png'
            im = filename

            if os.path.isfile(args.gt_path + split):
                gt_files.append(args.gt_path + split)
                im_sizes.append(cv2.imread(args.gt_path + split).shape[:2])
                im_files.append(args.gt_path + im)
                cams.append(2)
            else:
                num_probs += 1
                print('{} missing'.format(args.gt_path + im))
        print(str(num_probs) + ' files missing')
        ...
        for t_id in range(num_samples):
            ...
            depth = np.array(cv2.imread(gt_files[t_id],-1)).astype(np.float32)
            #crop front of the car in bottom 1/5
            crop_height = (im_sizes[t_id][0] * 4) // 5
            depth = depth[:crop_height,:]
            depth = cv2.resize(depth, (im_sizes[t_id][1], im_sizes[t_id][0]), interpolation=cv2.INTER_LINEAR)
            depth = (depth - 1) / 256.
            gt_depths.append(depth.astype(np.float32))
            #resize predicted depth
        ...
        num_samples = 697
        test_files = read_text_lines('/data1/pilzer/monodepth-GAN/utils/filenames/eigen_test_files_png.txt')
    ...
    for i in range(num_samples):
        ...
        if args.split == 'cityscapes':
            mask = np.logical_and(gt_depth > (1/args.max_depth), gt_depth < 125.)
